...
void HostInstance::UI_VolatileTasks()
{
                ...
                if (opened[n] && ImGui::BeginTabItem(names[n], &opened[n], ImGuiTabItemFlags_None))
                {
                    if (opened[n] && n == 0)
                    {
                        ...
                        if (selected == 0)
                        {
                            ...
                            for (auto chaine : this->host->tasks)
                            {
                            ...
                            {
                                std::string label = "packhhageView###" + std::to_string(row) + this->host->packages[row]->label + std::to_string(i);
                                ImGuiID id = ImGui::GetID(label.c_str());
                                ImGui::BeginChildFrame(id, ImVec2(0, 150), true);

                                // Affichage des éléments individuels
                                std::string ll = this->host->packages[row]->label;
                                ImGui::Text(ll.c_str());
                                ...
                                if (ImGui::Button("Select"))
                                {
                                    ...
                                    for (auto task : this->host->tasks)
                                    {
                                        if (task->tasktype == items[item_current])
                                        {
                                            ...
                                            _task->init();
                                            props->add("toolchain", this->host);
                                            props->add("package", this->host->packages[row]);

                                            _task->id = task->tasktype + "-" + VortexMaker::gen_random(8);
                                            _task->tasktype = task->tasktype;
                                            _task->component = this->host->packages[row]->label;
                                                ...
                                                if (ImGui::Button("Build"))
                                                {
                                                    ...
                                                    for (auto task : this->host->tasks)
                                                    {
                                                        if (task->tasktype == items[item_current])
                                                        {
                                                            ...
                                                            if(this->host->taskProcessor){

                                                {
                                                    std::lock_guard<std::mutex> lock(this->host->taskProcessor->mutex);
                                                    this->host->taskProcessor->tasksToProcess.push_back(_task);
                                                }

                                                                this->host->currentLoadedSystem.executedTasks.push_back(_task);
                                                                this->host->packages[row]->latestTask = _task;
                        ...
                        if (selected == 3)
                        {
                            ...
                            {
                                std::string label = "packageVifghgfew###" + std::to_string(row) + this->host->tasklists[row]->label + std::to_string(i);
                                ImGuiID id = ImGui::GetID(label.c_str());
                                ImGui::BeginChildFrame(id, ImVec2(0, 220), true);

                                // Affichage des éléments individuels
                                std::string ll = this->host->tasklists[row]->label;
                                ImGui::Text(ll.c_str());
                                ...
                                if (ImGui::Button("Select"))
                                {
                                    for (auto task : this->host->tasks)
                                    {
                                        if (task->tasktype == items[item_current])
                                        {
                                            ...
                                            _task->component = this->host->packages[row]->label;
                                            _task->priority = task->priority;
                                            _task->props->add("toolchain", this->host);
                                            _task->props->add("packages", this->host->packages[row]);
                                            _task->props->add("tasklist", this->host->tasklists[row]);
                                            _task->state = "not_started";
                                                ...
                                                if (ImGui::Button("Build"))
                                                {
                                                    ...
                                                    for (auto task : this->host->tasks)
                                                    {
                                                        if (task->tasktype == items[item_current])
                                                        {
                                                            ...
                                                            std::shared_ptr<hArgs> props = std::make_shared<hArgs>();
                                                            props->add("toolchain", this->host);
                                                            props->add("packages", this->host->packages[row]);
                                                            props->add("tasklist", this->host->tasklists[row]);
                    ...
                    if (opened[n] && n == 1)
                    {
                        ...
                        { // Ajout de la tâche aux listes appropriées
                            if (this->host->taskProcessor)
                            {
                                ...
                                {
                                    std::lock_guard<std::mutex> lock(this->host->taskProcessor->mutex);
                                    this->host->taskProcessor->tasksToProcess.push_back(selectedTask);
                                }
                                

                                this->host->currentLoadedSystem.executedTasks.push_back(selectedTask);
                                // this->host->packages[row]->latestTask = _task;
                                this->host->currentLoadedSystem.Save(this->host);
                            }
                            else
