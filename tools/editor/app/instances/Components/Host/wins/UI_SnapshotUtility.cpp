...
void HostInstance::UI_SnapshotUtility()
{

    if (this->show_UI_SnapshotUtility)
    {

        static std::string label = this->name + " - Snapshot Utility";
...
static ImTextureID toolIcon = this->m_SettingsIcon->GetImGuiTextureID(VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);
        ...
        if (ImGui::BeginTable("table_snap", 3, flags))
        {
            ...
            ImGui::TableHeadersRow();
             for (int row = 0; row < this->host->snapshots.size(); row++)
                {
                    static std::pair<char[128], char[128]> newItem;
                    static char label[128];
                    ...
                    for (int column = 0; column < 2; column++)
                    {
                        ...
                        if (column == 0)
                        {
                            if (ImGui::ImageButtonWithText(addIcon, "Delete", ImVec2(this->m_SaveIcon->GetWidth(), this->m_SaveIcon->GetHeight())))
                            {
                                this->host->MakeSnapshot(label);
                                this->Refresh();
                            }
                        ...
                        if (column == 1)
                        {
                            ImGui::Text(this->host->snapshots[row].name.c_str());
                        }
                        if (column == 2)
                        {
                            ImGui::Text(this->host->snapshots[row].snapshotSystem.size.c_str());
                        }
                    }
                }
        }
