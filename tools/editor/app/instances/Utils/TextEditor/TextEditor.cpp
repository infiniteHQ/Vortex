...
TextEditor::TextEditor(VxContext *ctx, std::string PathToFile)
	: mLineSpacing(1.0f)
	, mUndoIndex(0)
	, mTabSize(4)
	, mOverwrite(false)
	...
	, mStartTime(std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::system_clock::now().time_since_epoch()).count())
{
    ...
    {
        uint32_t w, h;
        void *data = Walnut::Image::Decode(icons::i_textedit, icons::i_textedit_size, w, h);
        m_MainIcon = std::make_shared<Walnut::Image>(w, h, Walnut::ImageFormat::RGBA, data);
        free(data);
    }
