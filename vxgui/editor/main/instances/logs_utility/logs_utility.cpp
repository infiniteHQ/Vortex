...
static float filterbar_width_ = 250.0f;
...
namespace vxe {
  LogsUtilityAppWindow::LogsUtilityAppWindow(const std::string &name) {
    ...
    Cherry::Log::AddCallback([this](Cherry::Log::Level level, const std::string &msg) { AddLogEntry(level, msg); });
    ...
    app_window_->SetLeftBottombarCallback([this]() {
        ...
        switch (data->EventFlag) {
          case ImGuiInputTextFlags_CallbackCompletion: {
            if (!internal_matches.empty()) {
              int idx = (s_SelectionIdx >= 0 && s_SelectionIdx < (int)internal_matches.size()) ? s_SelectionIdx : 0;
          ...
          case ImGuiInputTextFlags_CallbackHistory: {
            ...
            if (navigating_history) {
              ...
              if (prev_pos != s_HistoryPos) {
                data->DeleteChars(0, data->BufTextLen);
                data->InsertChars(0, s_HistoryPos >= 0 ? s_CommandHistory[s_HistoryPos].c_str() : "");
              }
      ...
      ImGuiInputTextFlags input_flags = ImGuiInputTextFlags_EnterReturnsTrue | ImGuiInputTextFlags_CallbackCompletion |
      ...
      if (CherryGUI::InputTextWithHint(
              "##ConsoleInput", "Execute command...", s_InputBuffer, IM_ARRAYSIZE(s_InputBuffer), input_flags, callback)) {
    ...
    LogUtilityChild filterbar("RenderFiltersBar", [this]() { RenderFiltersBar(); });
    filterbar.Disable();
    filterbar.m_DefaultSize = filterbar_width_;
    filterbar.m_BackgroundColor = Cherry::HexToRGBA("#35353535");
    AddChild(filterbar);

    LogUtilityChild contentbar("RenderContentBar", [this]() { RenderContentBar(); });
    contentbar.Enable();
  ...
  void LogsUtilityAppWindow::RenderFiltersBar() {
    ...
    if (CherryGUI::ImageSizeButtonWithText(
            Cherry::GetTexture(GetPath("resources/imgs/icons/misc/icon_warning.png")),
            header_width,
            "Warnings",
            ImVec2(-FLT_MIN, 0.0f),
            ImVec2(0, 0),
    ...
    CherryGUI::PushStyleColor(ImGuiCol_ButtonActive, HexToRGBA("#454545"));
    if (CherryGUI::ImageSizeButtonWithText(
            Cherry::GetTexture(GetPath("resources/imgs/icons/misc/icon_error.png")),
            header_width,
            "Errors",
            ImVec2(-FLT_MIN, 0.0f),
            ImVec2(0, 0),
    ...
    CherryGUI::PushStyleColor(ImGuiCol_ButtonActive, HexToRGBA("#454545"));
    if (CherryGUI::ImageSizeButtonWithText(
            Cherry::GetTexture(GetPath("resources/imgs/icons/misc/icon_fatal.png")),
            header_width,
            "Fatals",
            ImVec2(-FLT_MIN, 0.0f),
            ImVec2(0, 0),
    ...
    for (auto &[topic, isActive] : m_TopicsFilterStates) {
      ...
      if (CherryGUI::ImageSizeButtonWithText(
              Cherry::GetTexture(GetPath("resources/imgs/icons/misc/icon_journal.png")),
              header_width,
              topic.c_str(),
              ImVec2(-FLT_MIN, 0.0f),
              ImVec2(0, 0),
    ...
    if (show_mode_ == ShowMode::Advanced) {
      ...
      if (CherryGUI::BeginTable("LogsTable", 4, flags)) {
        ...
        bool hasActiveFilter = std::any_of(
            m_TopicsFilterStates.begin(), m_TopicsFilterStates.end(), [](const auto &pair) { return pair.second; });
        ...
        for (auto log : vxe::GetCurrentContext()->registered_logs) {
          ...
          if (m_UseWarningFilter || m_UseErrorFilter || m_UseFatalFilter || m_UseInfoFilter) {
  ...
  void LogsUtilityAppWindow::AddLogEntry(Cherry::Log::Level level, const std::string &msg) {
    if (!s_Entries.empty() && s_Entries.back().message == msg && s_Entries.back().level == level) {
