...
public:
  InstanceWindow(VxContext *ctx) { this->m_ctx = ctx; };
  ...
  void render() {
    ...
    if (ImGui::Begin(this->host->name.c_str(), &this->opened,
...
*/
  ...
  void menubar() {
    /*
       ...
       this->m_SettingsIcon->GetImGuiTextureID(VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);
                    ...
                    if(ImGui::ImageButtonWithText(saveIcon, "Save",
       ImVec2(this->m_SaveIcon->GetWidth(), this->m_SaveIcon->GetHeight()))){
                        //Save behavior
                    }
       ...
       this->m_FolderIcon->GetHeight()))){
                    ...
                    {
                        if (ImGui::MenuItem("Parameters window", NULL,
       &this->show_UI_ParametersWindow))
                        {
                        }
                        if (ImGui::MenuItem("Content window", NULL,
       &this->show_UI_ContentWindow))
                        {
                        }
                        ImGui::EndMenu();
                    }
                    ...
                    {
                        if (ImGui::MenuItem("Build/Rebuild single parts", NULL,
       &this->show_UI_ContentWindow))
                        {
                        }
                        if (ImGui::MenuItem("Global build"))
                    ...
                    if(ImGui::ImageButtonWithText(addIcon, "Add",
       ImVec2(this->m_AddIcon->GetWidth(), this->m_AddIcon->GetHeight()))){
                        //Save behavior
                    }
                    ...
                    if(ImGui::ImageButtonWithText(settingsIcon, "Settings",
       ImVec2(this->m_SettingsIcon->GetWidth(),
       this->m_SettingsIcon->GetHeight()))){
                        //Save behavior
                    }
                    ImGui::EndMenuBar();
                }
  ...
  void UI_ContentWindow() {
    if (this->show_UI_ContentWindow) {
      ...
      ImGuiTableColumnFlags_WidthStretch); ImGui::TableHeadersRow(); for (int
      row = 0; row < 5; row++)
              {
                  ImGui::TableNextRow();
                  for (int column = 0; column < 3; column++)
                  {
                      ...
                      ImGui::Text("%s %d,%d", (column == 2) ? "Stretch" :
      "Fixed", column, row);
                  }
              }
              ImGui::EndTable();
