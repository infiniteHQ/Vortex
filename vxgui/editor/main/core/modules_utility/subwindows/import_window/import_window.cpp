//
//  import_window.cpp
//  Sources for the "import window" of the modules utility
//
...
namespace vxe {
  ...
  ImportWindow::ImportWindow(const std::string &name) {
  ...
  std::shared_ptr<Cherry::AppWindow> &ImportWindow::get_app_window() {
    return app_window_;
  }

  std::shared_ptr<ImportWindow> ImportWindow::create(const std::string &name) {
    auto instance = std::shared_ptr<ImportWindow>(new ImportWindow(name));
    instance->setup_render_callback();
    return instance;
  }

  void ImportWindow::setup_render_callback() {
