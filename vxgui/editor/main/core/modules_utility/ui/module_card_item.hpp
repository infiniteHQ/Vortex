  ...
  class ModuleCardItem : public Cherry::Component {
   public:
    ...
    void Render() override {
      ...
      std::string truncatedDesc = module_->description();
      ...
      if (CherryGUI::CalcTextSize(originalDesc).x > max_text_width_) {
        size_t len = module_->description().size();

        size_t firstPart = std::min<size_t>(90, len);
        truncatedDesc = module_->description().substr(0, firstPart);
        ...
        if (CherryGUI::CalcTextSize(truncatedDesc.c_str()).x > max_text_width_) {
          ...
          truncatedDesc = module_->description().substr(0, firstLine);
          if (secondLine > 0) {
            truncatedDesc += "\n" + module_->description().substr(55, secondLine);
          }
        }
      } else {
        truncatedDesc = module_->description() + "\n";
      }
      ...
      if (CherryKit::ButtonImage(Cherry::GetPath("resources/imgs/icons/misc/icon_trash.png")).GetDataAs<bool>("isClicked")) {
        ...
        if (module_deletion_callback_) {
          module_deletion_callback_(module_);
        }
      }
      ...
      if (CherryKit::ButtonImage(Cherry::GetPath("resources/imgs/icons/misc/icon_settings.png"))
              .GetDataAs<bool>("isClicked")) {
        std::string label = "Details of " + module_->proper_name() + "####" + path_;
        std::shared_ptr<ModuleDetails> window = ModuleDetails::Create(label, module_);
   ...
   private:
    ...
    std::shared_ptr<ModuleInterface> module_;
    std::string name_;
    std::string path_;
