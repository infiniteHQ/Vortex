...
namespace vxe {
  ...
  PluginsUtility::PluginsUtility(const std::string &name) {
    ...
    app_window_->SetIcon(Cherry::GetPath("resources/imgs/icons/misc/icon_plugin.png"));
          ...
          if (m_SelectedShowMode == ShowModes::Thumbmails) {
            for (int i = 0; i < vxe::GetCurrentContext()->IO.ep.size(); i++) {
              ...
              if (!m_SelectedCategory.empty() && m_SelectedCategory != "all") {
                if (m_SelectedCategory != vxe::GetCurrentContext()->IO.ep[i]->m_group) {
                  continue;
                }
              }
              ...
              if (!PluginsSearch.empty()) {
                if (!HasCommonSubsequence(vxe::GetCurrentContext()->IO.ep[i]->m_name, PluginsSearch) &&
                    !HasCommonSubsequence(vxe::GetCurrentContext()->IO.ep[i]->m_proper_name, PluginsSearch) &&
                    !HasCommonSubsequence(vxe::GetCurrentContext()->IO.ep[i]->m_description, PluginsSearch)) {
                  continue;
                }
              }
              ...
              auto item = PluginCard(
                  ...
                  vxe::GetCurrentContext()->IO.ep[i]->m_name,
                  vxe::GetCurrentContext()->IO.ep[i]->m_version,
                  false,
                  vxe::GetCurrentContext()->IO.ep[i]->m_logo_path,
            ...
            if (!m_SelectedCategoryChanged) {
              CherryKit::GridSimple(CherryID("PluginUtilityGrid"), 250.0f, 250.0f, plugins_blocks);
            } else {
  ...
  PluginsUtilityChild *PluginsUtility::GetChild(const std::string &child_name) {
    auto it = std::find_if(childs_.begin(), childs_.end(), [&child_name](const PluginsUtilityChild &child) {
          ...
          "##body", ImVec2(MODAL_W, 200.0f), false, ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_AlwaysAutoResize);
      {
        ...
        CherryGUI::BeginChild("##plugincard", ImVec2(MODAL_W - 40.0f, 82.0f), false, ImGuiWindowFlags_NoScrollbar);
        {
          ...
          if (logo) {
            ...
            std::string initials = g_PluginToDeleteProperName.size() >= 2 ? g_PluginToDeleteProperName.substr(0, 2) : "??";
        ...
        if (CherryGUI::Button("Delete plugin", ImVec2(BTN_W, BTN_H))) {
          vxe::DeleteProjectPlugin(g_PluginToDeleteName, g_PluginToDeleteVersion);
