...
//  Sources for the "flash link window" of the plugin utility
//
...
namespace vxe {
  ...
  PluginFetchResult fetch_plugin_from_flashlink(const std::string &flashlink) {
    ...
    std::string plugin_url = "http://api.infinite.si:9000/api/garagevortex/get_plugin?uuid=" + uuid;
    std::string plugin_body;
    try {
      plugin_body = ctx.net.GET(plugin_url);
    } catch (...) {
      result.error = "Unable to fetch the service API";
      return result;
    }

    if (plugin_body.empty()) {
    ...
    try {
      plugin_json = nlohmann::json::parse(plugin_body);
    } catch (...) {
    ...
    PluginInfo info;
    info.uuid = plugin_json.value("uuid", "");
    info.name = plugin_json.value("name", "");
    info.proper_name = plugin_json.value("proper_name", "");
    info.description = plugin_json.value("description", "");
    info.picture_link = plugin_json.value("picture_link", "");
    info.banner_link = plugin_json.value("banner_link", "");
    info.host = plugin_json.value("host", "");
    info.host_link = plugin_json.value("host_link", "");
    info.certified = plugin_json.value("certified", false);
    info.state = plugin_json.value("state", "");

    result.info = info;

    std::string releases_url =
        "https://api.infinite.si/api/garagevortex/get_content_releases_summary?parent_uuid=" + uuid + "&kind=plugin";
    ...
    std::vector<PluginRelease> parsed;
    ...
    std::vector<PluginRelease> cross;
    for (auto &rel : parsed) {
      if (rel.platform == "cross") {
        cross.push_back(rel);
      }
    }
    std::sort(cross.begin(), cross.end(), [](const PluginRelease &a, const PluginRelease &b) {
      return version_greater(a.version, b.version);
    });

    std::vector<PluginRelease> compatible;
    for (auto &rel : parsed) {
    ...
    std::sort(compatible.begin(), compatible.end(), [](const PluginRelease &a, const PluginRelease &b) {
      return version_greater(a.version, b.version);
    });
  ...
  FlashLinkPluginWindow::FlashLinkPluginWindow(const std::string &name, const std::string &mode) {
  ...
  std::shared_ptr<Cherry::AppWindow> &FlashLinkPluginWindow::get_app_window() {
    return app_window_;
  }

  std::shared_ptr<FlashLinkPluginWindow> FlashLinkPluginWindow::create(const std::string &name, const std::string &mode) {
    auto instance = std::shared_ptr<FlashLinkPluginWindow>(new FlashLinkPluginWindow(name, mode));
    instance->setup_render_callback();
    return instance;
  }

  void FlashLinkPluginWindow::setup_render_callback() {
  ...
  void FlashLinkPluginWindow::startSearch(const std::string &flashlink) {
    ...
    std::thread([this, my_token, flashlink]() {
      PluginFetchResult res = vxe::fetch_plugin_from_flashlink(flashlink);
  ...
  bool FlashLinkPluginWindow::tryProcessCandidate(const std::string &raw, bool clear_input_on_success) {
    ...
    if (!is_plugin_flashlink(decoded)) {
      return false;
    }
  ...
  void FlashLinkPluginWindow::renderCloseButton() {
    ImGui::Spacing();
    ImGui::Separator();
    ImGui::Spacing();
  ...
  void FlashLinkPluginWindow::render() {
    ...
    if (mode_ == "flash") {
      ...
      ImGui::TextUnformatted("Please copy plugin flash code");
      ImGui::Spacing();
      ...
      if (ImGui::Button("Search", ImVec2(80.0f, 0.0f))) {
        ...
        if (!ok) {
          manual_search_error_ = "Invalid flashlink code. Please copy a valid plugin flashlink.";
        }
      }
    ...
    switch (current_state) {
      case FlashLinkState::WaitingForClipboard: {
        if (mode_ == "flash") {
          ...
          ImGui::TextColored(ImVec4(0.7f, 0.7f, 0.7f, 1.0f), "Enter a plugin flashlink above and click Search.");
        }
        break;
      }
      ...
      case FlashLinkState::Ready: {
        PluginFetchResult local_result;
        {
          std::lock_guard<std::mutex> lock(result_mutex_);
        ...
        ImGui::BeginGroup();
        {
          ...
          ImGui::TextUnformatted(
              local_result.info.proper_name.empty() ? "Unknown plugin" : local_result.info.proper_name.c_str());
        ...
        if (!has_native && !has_cross) {
          ImGui::TextColored(
              ImVec4(1.0f, 0.75f, 0.3f, 1.0f), "No compatible version of this plugin found for your platform.");
          break;
        }
        ...
        if (has_cross && has_native) {
          ...
          if (ImGui::IsItemHovered()) {
            ImGui::SetTooltip(
                "Selecting a cross-platform plugin will take a bit more disk\n"
          ...
          selected_platform_mode_ = PlatformTargetMode::Cross;
          ImGui::TextDisabled("Cross-platform plugin (works on Linux, Windows and macOS)");
          if (ImGui::IsItemHovered()) {
            ImGui::SetTooltip(
                "This plugin is cross-platform: it takes a bit more disk\n"
                "space, but makes it easy to share the project with other\n"
                "platforms.");
          }
          ImGui::Spacing();
        ...
        const std::vector<PluginRelease> &active_releases = (selected_platform_mode_ == PlatformTargetMode::Cross)
        ...
        if (ImGui::Button("Download and install", ImVec2(avail_w, 0.0f))) {
          const PluginRelease &chosen = active_releases[selected_release_index_];

          std::string parent_uuid = local_result.info.uuid;
          std::string release_url =
              "https://api.infinite.si/api/garagevortex/get_release/plugin?parent_uuid=" + parent_uuid +
              "&uuid=" + chosen.uuid;
