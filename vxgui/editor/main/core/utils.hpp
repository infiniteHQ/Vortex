...
static std::vector<std::string> labels = {"Open a project",
...
static std::string default_project_avatar =
    "/usr/local/include/Vortex/imgs/base_vortex.png";
static std::string operating_system_banner =
    "/usr/local/include/Vortex/1.1/imgs/operating_system_banner.png";
static std::string _parent;

static char ProjectSearch[256];
static float threshold = 0.4;

static void saveVortexVersions(const std::vector<std::string> &paths,
...
static void loadVortexVersions(std::vector<std::string> &paths,
                               const std::string &jsonFilePath) {
...
static void saveTemplates(const std::vector<std::string> &paths,
                          const std::string &jsonFilePath) {
  nlohmann::json jsonData;
...
static void loadTemplates(std::vector<std::string> &paths,
                          const std::string &jsonFilePath) {
  std::ifstream file(jsonFilePath);
  if (file) {
    nlohmann::json jsonData;
...
static void saveModules(const std::vector<std::string> &paths,
                        const std::string &jsonFilePath) {
  nlohmann::json jsonData;
...
static void loadModules(std::vector<std::string> &paths,
                        const std::string &jsonFilePath) {
  std::ifstream file(jsonFilePath);
  if (file) {
    nlohmann::json jsonData;
...
static void saveProjects(const std::vector<std::string> &projectPaths,
                         const std::string &jsonFilePath) {
  nlohmann::json jsonData;
...
static void loadProjects(std::vector<std::string> &projectPaths,
                         const std::string &jsonFilePath) {
  std::ifstream file(jsonFilePath);
  if (file) {
    nlohmann::json jsonData;
...
static void MyButton(const std::string &name, int w, int h) {
  ...
  if (!name.empty()) {
    drawList->AddImage(
        Cherry::GetTexture(name), cursorPos,
        ImVec2(cursorPos.x + squareSize.x, cursorPos.y + squareSize.y));
  } else {
    drawList->AddImage(
        Cherry::GetTexture(Cherry::GetPath(
            "resources/imgs/icons/misc/icon_vortex_default.png")),
        cursorPos,
        ImVec2(cursorPos.x + squareSize.x, cursorPos.y + squareSize.y));
  }

  ImVec2 smallRectSize(40, 20);
  ImVec2 smallRectPos(cursorPos.x + squareSize.x - smallRectSize.x - 5,
                      cursorPos.y + squareSize.y - smallRectSize.y - 5);
...
static void DrawDescription(ImDrawList *drawList, ImVec2 textPos,
                            const char *text, const char *search,
                            ImU32 highlightColor, ImU32 textColor,
                            ImU32 highlightTextColor, float oldfontsize) {
...
static void VersionButton(const std::string &envproject, int xsize = 100,
                          int ysize = 100, const std::string &version = "?") {
  ...
  if (!envproject.empty() && std::filesystem::exists(envproject)) {
    drawList->AddImage(
        Cherry::GetTexture(envproject), cursorPos,
        ImVec2(cursorPos.x + squareSize.x, cursorPos.y + squareSize.y));
  } else {
    drawList->AddImage(
        Cherry::GetTexture(
            Cherry::GetPath("resources/imgs/vortex_banner_unknow.png")),
        cursorPos,
        ImVec2(cursorPos.x + squareSize.x, cursorPos.y + squareSize.y));
  }
  ...
  drawList->AddRectFilled(smallRectPos,
                          ImVec2(smallRectPos.x + smallRectSize.x,
                                 smallRectPos.y + smallRectSize.y),
                          IM_COL32(0, 0, 0, 255));
  ImVec2 versionTextPos =
      ImVec2(smallRectPos.x +
                 (smallRectSize.x - CherryGUI::CalcTextSize(versionText).x) / 2,
             smallRectPos.y +
                 (smallRectSize.y - CherryGUI::CalcTextSize("version").y) / 2);
  drawList->AddText(versionTextPos, IM_COL32(255, 255, 255, 255), versionText);

  ImVec2 textPos = ImVec2(cursorPos.x + (squareSize.x - textSize.x) / 2,
                          cursorPos.y + squareSize.y + 5);
  ...
  DrawHighlightedText(drawList, textPos, truncatedText, ProjectSearch,
      ...
      if (entry.is_directory()) {
        ...
        if (std::filesystem::exists(manifest_path)) {
          try {
            ...
            std::string image_path = base_path + "/" + version_dir + "/" +
                                     manifest_json["image"].get<std::string>();
            ...
            std::cerr << "Erreur lors de la lecture de " << manifest_path
                      << ": " << e.what() << std::endl;
          }
        }
      }
...
static void MyButton(const std::shared_ptr<EnvProject> envproject,
  ...
  std::string button_id =
      envproject->name + "squareButtonWithText" + envproject->lastOpened;
  if (CherryGUI::InvisibleButton(button_id.c_str(), totalSize)) {
    selected_envproject = envproject;
  }
  ...
  if (!envproject->logoPath.empty()) {
    drawList->AddImage(
        Cherry::GetTexture(envproject->logoPath), cursorPos,
        ImVec2(cursorPos.x + squareSize.x, cursorPos.y + squareSize.y));
  } else {
    drawList->AddImage(
        Cherry::GetTexture(Cherry::GetPath(
  ...
  if (CheckIfVortexVersionExist(envproject->compatibleWith)) {
    drawList->AddText(versionTextPos, IM_COL32(255, 255, 255, 255),
                      versionText);
  } else {
...
static void
MyButton(const std::shared_ptr<TemplateInterface> templateinterface) {
  ...
  drawList->AddImage(
      Cherry::GetTexture(templateinterface->m_logo_path), cursorPos,
      ImVec2(cursorPos.x + squareSize.x, cursorPos.y + squareSize.y));
  ...
  ImVec2 versionTextPos = ImVec2(
      smallRectPos.x +
          (smallRectSize.x -
           CherryGUI::CalcTextSize(templateinterface->m_group.c_str()).x) /
              2,
      smallRectPos.y +
          (smallRectSize.y - CherryGUI::CalcTextSize("version").y) / 2);
  drawList->AddText(versionTextPos, IM_COL32(255, 255, 255, 255),
                    templateinterface->m_group.c_str());
...
static void MyBanner(const std::string &path) {
  ...
  drawList->AddImage(
      Cherry::GetTexture(path), cursorPos,
      ImVec2(cursorPos.x + squareSize.x, cursorPos.y + squareSize.y));
