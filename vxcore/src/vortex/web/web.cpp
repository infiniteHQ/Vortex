...
std::string vxe::get_plugins_directory() {
  std::string path = vxe::get_current_context()->projectPath.string() + "/.vx/plugins";
  vxe::ensure_directory_exists(path);
  return path;
}

std::string vxe::get_plugin_install_temp_directory() {
#ifdef _WIN32
  std::string path = std::filesystem::temp_directory_path().string() + "\\vortex_plugin_install";
#else
  std::string path = "/tmp/vortex_plugin_install";
#endif
  vxe::ensure_directory_exists(path);
  return path;
}
...
void vxe::install_plugin_release_async(
    const nlohmann::json &release_json,
    const std::string &parent_uuid,
    std::shared_ptr<ModuleInstallProgress> progress) {
  if (!progress) {
  ...
  std::thread([release_json, parent_uuid, progress]() {
    ...
    if (!sum_found || expected_sum.empty()) {
      progress->set_error("No sha256 sum for this plugin files, cannot process.");
      return;
    }
    ...
    std::string pluginsDir = vxe::get_plugins_directory();
    std::string tempDir = vxe::get_plugin_install_temp_directory();

    if (pluginsDir.empty() || tempDir.empty()) {
      progress->set_error("Unable to prepare installation folder.");
      return;
    }
    ...
    if (filename.empty()) {
      filename = "plugin_download.tar.gz";
    }
    ...
    // extraction
    progress->state.store(ModuleInstallState::Extracting);
    progress->set_status("Extracting the plugin...");

    std::string extractError;
    if (!extract_tar(archivePath, pluginsDir, extractError)) {
      progress->set_error("Module extraction failed : " + extractError);
      return;
    }
    ...
    {
      std::lock_guard<std::mutex> lock(progress->mutex);
      progress->install_path = pluginsDir;
      progress->status_message = "Module installed in this project with success.";
    }
    progress->state.store(ModuleInstallState::Done);

    vxe::notify_plugin_download(parent_uuid);
  }).detach();
}
...
void vxe::notify_plugin_download(const std::string &parent_uuid) {
  try {
    nlohmann::json payload;
    payload["uuid"] = parent_uuid;

    vxe::get_current_context()->net.POST("https://api.infinite.si/api/garagevortex/download_plugin", payload.dump());
  } catch (...) {
    // no error needed
  }
}