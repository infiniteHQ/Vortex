//
//  refresh.cpp
//  Sources of features related to project refreshing and informations refreshing
//
...
VORTEX_API void vxe::refresh_project_informations() {
  ...
  if (file) {
    try {
      ...
      vxe::log_error("Core", std::string("Failed to parse vortex.config JSON: ") + e.what());
      return;
    }
    // vxe::init_project(jsonContent);
  } else {
    vxe::log_error("Core", "Failed to open vortex.config file");
    return;
  }

  if (!jsonContent.contains("project") || !jsonContent["project"].is_object()) {
    vxe::log_error("Core", "vortex.config missing valid 'project' section");
    return;
  }
  ...
  if (project.contains("compatibleWith") && project["compatibleWith"].is_string())
    ctx->compatibleWith = project["compatibleWith"].get<std::string>();

  if (project.contains("include_system_templates") && project["include_system_templates"].is_boolean())
  ...
  if (project.contains("support_contact") && project["support_contact"].is_string())
    ctx->support_contact = project["support_contact"].get<std::string>();

  if (project.contains("copyright_file") && project["copyright_file"].is_string())
  ...
  if (project.contains("requirements_file") && project["requirements_file"].is_string())
    ctx->requirements_file = project["requirements_file"].get<std::string>();

  if (project.contains("code_of_conduct_file") && project["code_of_conduct_file"].is_string())
  ...
  if (project.contains("root_content_path") && project["root_content_path"].is_string())
