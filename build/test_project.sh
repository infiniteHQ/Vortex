...
cd "$PROJECT_PATH" && bash "$VORTEX_PATH/handle_crash.sh" "~/.vx/sessions/${SESSION_ID}/crash/core_dumped.txt" valgrind "$VORTEX_PATH/vortex --editor --session_id="${SESSION_ID}""